...
class Drone:
    ...
    def _encontrar_punto_frontera(self, grilla_cobertura, tamano_celda, num_celdas_x, num_celdas_y):
        """
        Lógica MVP para encontrar un punto frontera.
        Busca la celda no cubierta más cercana dentro de un radio de búsqueda.
        """
        mejor_punto = None
        ...
        for dx in range(-radio_busqueda_celdas, radio_busqueda_celdas + 1):
            for dy in range(-radio_busqueda_celdas, radio_busqueda_celdas + 1):
                if dx == 0 and dy == 0: # No considerar la celda actual del dron
                    continue
                ...
                if 0 <= celda_x_actual < num_celdas_x and \
                   0 <= celda_y_actual < num_celdas_y:
                    if grilla_cobertura[celda_x_actual, celda_y_actual] == 0: # Si la celda no está cubierta
                        # Punto central de esta celda no cubierta
        ...
        # Si no se encontró ninguna celda no cubierta cercana, podría moverse aleatoriamente o hacia el centro
        if mejor_punto is None:
             # Moverse a un punto aleatorio en el mapa como último recurso de exploración
             mejor_punto = np.array([random.uniform(0, config.ANCHO_PANTALLA), 
                                     random.uniform(0, config.ALTO_PANTALLA)])
             print(f"Dron {self.id}: No encontró frontera cercana, objetivo aleatorio {mejor_punto}")
        # else:
            # print(f"Dron {self.id}: Frontera encontrada en {mejor_punto}")
        return mejor_punto
    ...
    def calcular_fuerzas(self, otros_drones, obstaculos, grilla_cobertura, tamano_celda, num_cx, num_cy):
        ...
        # --- 1. Fuerza de Atracción a la Frontera (nueva lógica) ---
        ...
        # (Esta sección es la misma que en Etapa 2, solo nos aseguramos que usa config.K_COHESION, etc.)
        ...
        # (Esta sección es la misma que en Etapa 2)
        fuerza_repulsion_obstaculos = np.array([0.0, 0.0])
        for obs in obstaculos:
            if obs.esta_activo: # Solo considerar obstáculos activos
    ...
    def actualizar_estado_simple(self, nueva_posicion, nueva_velocidad):
        # ... (sin cambios respecto a Etapa 3) ...
